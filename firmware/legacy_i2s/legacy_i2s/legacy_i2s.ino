...
constexpr uint16_t FRAME_SIZE =
    SAMPLE_RATE * FRAME_DURATION_MS / 1000;   // 320 samples
...
// Buffers
// =====================================================
// Scratch buffer for the raw 32-bit I2S read. Never sent
// over serial directly.
static int32_t i2sReadBuffer[FRAME_SIZE];

// Transmit buffer: 4-byte marker + 16-bit samples.
// Sending int16 instead of int32 HALVES the required serial
// bandwidth (1284 bytes/frame -> 644 bytes/frame), which is
// what makes real-time streaming actually fit inside a 20ms
// frame period at a stable baud rate.
...
void loop()
{
    ...
    {
        // >>8 strips I2S byte padding (24-bit left-justified data).
        // >>8 again compresses the 24-bit range down to 16-bit.
